...
void  create_png_texture(const char *filename,GLTEXTURE2D* gltex2d,unsigned int _screen_width,unsigned int _screen_height);
void  create_compressed_texture(const char *filename,GLTEXTURE2D* gltex2d,unsigned int _screen_width,unsigned int _screen_height);
...
void  create_compressed_texture(const char *filename,GLTEXTURE2D* gltex2d,unsigned int _screen_width,unsigned int _screen_height){
	...
	GLubyte *pixels;
	loadData(filename,&pixels);
	GL_CHECK(glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_ETC1_RGB8_OES,
		getWidth((ETCHeader *)pixels), getHeight((ETCHeader *)pixels), 0,
		(getPaddedWidth((ETCHeader *)pixels) * getPaddedHeight((ETCHeader *)pixels)) >> 1,
		pixels + sizeof(ETCHeader)));
	gltex2d->image_width  = getWidth((ETCHeader *)pixels);
	gltex2d->image_height = getHeight((ETCHeader *)pixels);
#ifdef _DEBUG
	printf("Compressed Image Name :%s, Size : width : %d,height : %d \n",filename,gltex2d->image_width,gltex2d->image_height);
#endif
	free(pixels);
